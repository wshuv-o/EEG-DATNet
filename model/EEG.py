...
def attention(query, key, value, scale_factor=1.0):
    dim = query.size(-1)
    scores = torch.einsum('bhqd,bhkd->bhqk', query, key) / (dim**0.5 * scale_factor)  # Slightly modified scoring
    attn = F.softmax(scores, dim=-1)
    out = torch.einsum('bhqk,bhkd->bhqd', attn, value)
    return out, attn

class NovelMultiHeadedAttention(nn.Module):
    def __init__(self, d_model, n_head, dropout, scale_factor=1.0):
        super().__init__()
        self.d_k = d_model // n_head
        self.d_v = d_model // n_head
        self.n_head = n_head
        self.scale_factor = scale_factor  # New parameter for scaling
    ...
    def forward(self, query, key, value):
        ...
        out, _ = attention(q, k, v, self.scale_factor)  # Use modified attention
...
class NovelFeedForward(nn.Module):
    def __init__(self, d_model, d_hidden, dropout):
        super().__init__()
        self.w_1 = nn.Linear(d_model, d_hidden)
        self.act = nn.SiLU()  # Changed activation from GELU to SiLU (Novel Activation)
        self.w_2 = nn.Linear(d_hidden, d_model)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        x = self.w_1(x)
        x = self.act(x)  # Using SiLU activation
        x = self.dropout(x)
        x = self.w_2(x)
        x = self.dropout(x)
...
class NovelTransformerEncoder(nn.Module):
    def __init__(self, embed_dim, num_heads, fc_ratio, attn_drop=0.5, fc_drop=0.5, scale_factor=1.1):
        super().__init__()
        self.multihead_attention = NovelMultiHeadedAttention(embed_dim, num_heads, attn_drop, scale_factor)
        self.feed_forward = NovelFeedForward(embed_dim, embed_dim * fc_ratio, fc_drop)
        self.layernorm1 = nn.LayerNorm(embed_dim)
        self.layernorm2 = nn.LayerNorm(embed_dim)
        self.extra_norm = nn.LayerNorm(embed_dim)  # Added extra layer normalization

    def forward(self, data):
        res = self.layernorm1(data)
        out = data + self.multihead_attention(res, res, res)  # Residual connection remains intact

        res = self.layernorm2(out)
        output = out + self.feed_forward(res)
        
        # Added additional layer normalization after the feed-forward network
        output = self.extra_norm(output)
...
class NeuroTransNet(nn.Module):
    ...
    pool_stride=15, num_heads=8, fc_ratio=4, depth=4, attn_drop=0.5, fc_drop=0.5):
        ...
        self.transformer_encoders = nn.ModuleList(
            [NovelTransformerEncoder(embed_dim, num_heads, fc_ratio, attn_drop, fc_drop, scale_factor=1.1) for _ in range(depth)]
        )
