...
# Debugging NeuroTransNetEEGTH Model
class NeuroTransNetEEGTH(nn.Module):
    def __init__(self, num_classes=4, num_samples=1000, num_channels=22, embed_dim=32, pool_size=50, 
        ...
        # Calculate the exact output size of the pooling layers
        self.temp_embedding_dim = (num_samples - pool_size) // pool_stride + 1
        print(f"Calculated temp_embedding_dim: {self.temp_embedding_dim}")
        ...
        self.conv_encoder = nn.Sequential(
            nn.Conv2d(9, 64, (2, 1)),  # This affects the output shape
            nn.BatchNorm2d(64),
            nn.ELU()
        )
        
        # For now, use 1024 as the input dimension for the classification layer
        # We'll modify this after seeing the actual tensor shape in the forward pass
        self.classify = nn.Linear(1024, num_classes)
        
        print(f"Classification layer input: 1024, output: {num_classes}")
        
        # Flag to indicate first forward pass for debugging
    ...
    def forward(self, x):
        ...
        if self.first_forward:
            print(f"After temp_conv and cat: {x.shape}")
        
        x = self.bn1(x)
        x = self.spatial_conv(x)
        if self.first_forward:
            print(f"After spatial_conv: {x.shape}")
        ...
        x2 = self.var_pool(x)
        if self.first_forward:
            print(f"After pooling - x1: {x1.shape}, x2: {x2.shape}")
        ...
        if self.first_forward:
            print(f"After rearrange - x1: {x1.shape}, x2: {x2.shape}")
        ...
        if self.first_forward:
            print(f"After transformer - x1: {x1.shape}, x2: {x2.shape}")
        
        x1 = x1.unsqueeze(dim=2)
        x2 = x2.unsqueeze(dim=2)
        if self.first_forward:
            print(f"After unsqueeze - x1: {x1.shape}, x2: {x2.shape}")
        
        # Debug shape for cat
        if self.first_forward:
            print(f"Shapes before cat - x1: {x1.shape}, x2: {x2.shape}, dim=2")
        ...
        if self.first_forward:
            print(f"After conv_encoder: {x.shape}")
        ...
        if self.first_forward:
            ...
            # Adjust the classification layer to match the actual input dimension
            actual_dim = x.shape[1]
            print(f"Actual flatten dimension: {actual_dim}")
            
            # Create a new classification layer with the correct input dimension
            if actual_dim != 1024:
                print(f"WARNING: Actual dimension {actual_dim} doesn't match expected 1024")
                print(f"You should update your model definition with the correct dimension")
