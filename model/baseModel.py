...
class baseModel():
    ...
    def data_augmentation(self, data, label):
        ...
        N, C, T = data.shape  # N: number of trials, C: number of channels, T: number of time points
        seg_size = T // self.num_segs  # Segment size based on num_segs
        aug_data_size = self.batchsize // self.num_classes  # Augmented data size per class
        ...
        for cls in range(self.num_classes):
            ...
            # Prepare temporary augmented data array for this class
            temp_aug_data = np.zeros((aug_data_size, C, T))

            for i in range(aug_data_size):
                rand_idx = np.random.randint(0, data_size, self.num_segs)  # Random indices for augmentation
                for j in range(self.num_segs):
                    temp_aug_data[i, :, j * seg_size:(j + 1) * seg_size] = cls_data[rand_idx[j], :, j * seg_size:(j + 1) * seg_size]

            aug_data.append(temp_aug_data)  # Append augmented data for this class
            aug_label.extend([cls] * aug_data_size)  # Append labels for the augmented data

        # Check if aug_data is empty before attempting to concatenate
        if len(aug_data) == 0:
            print("No augmented data generated. Skipping augmentation.")
            return data, label  # Return the original data and label if no augmentation

        # Concatenate the augmented data for all classes
        aug_data = np.concatenate(aug_data, axis=0)
        aug_label = np.array(aug_label)
        ...
        aug_data = torch.from_numpy(aug_data).to(self.device)  # Ensure data is on the correct device
        aug_label = torch.from_numpy(aug_label).to(self.device)  # Ensure labels are on the correct device

        return aug_data, aug_label

    def train_test(self, train_dataset, test_dataset):
        train_dataloader = DataLoader(train_dataset, batch_size=self.batchsize, shuffle=True, num_workers=1, pin_memory=True)
        test_dataloader = DataLoader(test_dataset, batch_size=self.batchsize, num_workers=1, pin_memory=True)
