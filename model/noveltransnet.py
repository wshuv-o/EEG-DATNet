...
class TransNetNovel(nn.Module):
    def __init__(self, num_classes=4, num_samples=1000, num_channels=22, embed_dim=32, pool_size=50, 
        ...
        # Convolutional layers to process temporal features
        ...
        # Hybrid Pooling (mixing average and variance pooling)
        self.var_pool = nn.AdaptiveAvgPool1d(pool_size)
        ...
        # Adjusted depth of transformer encoders for dynamic computation
        self.transformer_encoders = nn.ModuleList(
            [TransformerEncoder(embed_dim, num_heads, fc_ratio, attn_drop, fc_drop) for i in range(depth)]
        )

        self.conv_encoder = nn.Sequential(
            nn.Conv2d(122, 64, (2, 1)),  # New input channels from concatenation
            nn.BatchNorm2d(64),
            nn.ELU()
        )
    ...
    def forward(self, x):
        ...
        # Step 5: Reshape and Classification
